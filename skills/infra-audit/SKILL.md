...
- CI workflow installs Playwright browsers (`playwright install` appears in any step) **with the `--with-deps` flag** — on `ubuntu-latest`, the required system libraries (libnss3, libxss1, libatk-bridge2.0-0, etc.) are already pre-installed, so `--with-deps` adds ~30s of redundant `apt-get install` and requires `sudo`, which some hardened runners reject. Remove `--with-deps` unless the job runs on a bare base image (e.g. a custom Dockerfile from Alpine). Detection: `grep -E "playwright\s+install.*--with-deps"` across all `.github/workflows/*.yml` files
- CI workflow installs Playwright browsers but **has no `actions/cache` step** targeting `~/.cache/ms-playwright` (or equivalent `PLAYWRIGHT_BROWSERS_PATH` override) — every CI run redownloads 150-300 MB of Chromium (~15-30s per job, plus occasional hangs when Playwright's CDN is flaky). Detection: if `grep -l "playwright install"` matches any workflow file AND the same file does not contain both `uses: actions/cache` and `ms-playwright` within a reasonable window, flag it. See blueprint §4 "Playwright browser caching" for the reference pattern
- CI workflow caches Playwright browsers but uses `actions/cache` with a **key that does not include a lockfile hash** (e.g. a static string like `playwright-chromium` or a key based only on `runner.os`) — the cache never invalidates when Playwright itself is bumped, so the old browser version sticks around until someone manually bumps the key. Expected shape: `key: playwright-${{ runner.os }}-${{ hashFiles('<lockfile>') }}`
- CI runs both a Python Playwright job (via `pytest-playwright` / `.venv/bin/playwright install`) and a Node Playwright job (via `pnpm/npm/yarn exec playwright install`) but the two jobs use **different cache keys** — the cache is scoped per repo, so using the same key lets the second job hit a ~2s cache restore from the first job's population. Different keys mean each job pays the cold-download cost independently (INFO-level if one uses caching and the other doesn't; WARNING if both use caching with mismatched keys)
